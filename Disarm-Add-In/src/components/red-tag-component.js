...
import {
  chooseTechniquesPopup,
  phaseChooseField,
  tacticChooseField,
  techniqueChooseCheckboxField,
  searchTechniquesPopup,
  phaseSearchField,
  tacticSearchField,
  checkbox,
  textBox,
  listSearchTechniquesPopup,
  alertSearchBox,
  table,
...
$(document).on("click", "tr", function (e) {
  handleTableColorOnClick.call(this);
});
  ...
  Object.keys(tacticsArray).forEach((tactic, index) => {
    ...
    if (insertTag) {
      row.addEventListener("click", function () {
        helpersUI.showTechniqueOptions(tacticsArray[tactic], tags);
      });
    }
...
export async function saveButtonInsertTechniques() {
  await Word.run(async (context) => {
    ...
    var sentenceRange = await context.sync().then(() => {
      return sentenceInitialRange.items[0];
    });
